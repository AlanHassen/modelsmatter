...
from aizynthfinder.context import policy
...
from aizynthfinder.context.policy.expansion_strategies import (
    ExpansionStrategy,
    TemplateBasedExpansionStrategy, SmilesBasedExpansionStrategy,
)
...
class ExpansionPolicy(ContextCollection):
    ...
    def load_from_config(self, **config: Any) -> None:
        ...
        files_spec = config.get("files", config.get("smiles-based", {}))
        for key, policy_spec in files_spec.items():
            dataFile = policy_spec
            strategy = SmilesBasedExpansionStrategy(
                key, self._config, data=dataFile
            )
            self.load(strategy)
        ...
        for strategy_spec, strategy_config in config.items():
            if strategy_spec in ["files", "template-based", "smiles-based"]:
                continue
