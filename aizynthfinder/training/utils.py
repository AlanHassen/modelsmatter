...
def split_and_save_data_random(
...
def split_and_save_data(data: Union[pd.DataFrame, np.ndarray, sparse.csr_matrix],data_label: str,config: Config, used_split: pd.Series) -> None:
    """Split based on a split column and save the data

    Args:
        data (Union[pd.DataFrame, np.ndarray, sparse.csr_matrix]): _description_
        data_label (str): _description_
        config (Config): _description_
        used_split (pd.Series): _description_
    """
    # select train based on used_split
    train_arr = data[used_split == "train"]
    val_arr = data[used_split == "valid"]

    # assert that train_arr and val_arr are not empty
    assert train_arr.shape[0] > 0
    assert val_arr.shape[0] > 0

    array_dict = {"training_": train_arr, "validation_": val_arr}

    for label_prefix, arr in array_dict.items():
        filename = config.filename(label_prefix + data_label)
        if isinstance(data, pd.DataFrame):
            print(f"{filename}: {arr}", flush=True)
            arr.to_csv(
                filename,
                sep=config["csv_sep"],
                header=config["in_csv_headers"],
...
def save_data_with_defined_split(data: Union[pd.DataFrame, np.ndarray, sparse.csr_matrix],data_label: str,config: Config, label_prefix: str) -> None:
    """Split based on a split column and save the data
